...
class Options:
    ...
    def parse_text(text: str, before: str, after: str) -> str:
        try:
            return re.search(rf'{before}(.*?){after}', text).group(1).strip().rstrip()
        except:
            return ""

class Recaptcha3Pypass:
    def __init__(self, anchor_url: str, reload_url: str, timeout: int = 20):
        ...
        # استخراج القيم بنفس المنطق الأصلي
        self.v = Options.parse_text(self.anchor_url, 'v=', '&')
        self.k = Options.parse_text(self.anchor_url, 'k=', '&')
        self.co = Options.parse_text(self.anchor_url, 'co=', '&')
